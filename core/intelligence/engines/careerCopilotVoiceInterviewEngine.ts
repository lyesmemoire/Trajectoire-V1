...
export class CareerCopilotVoiceInterviewEngine {
  ...
  static startInterview(
    ...
    const conversationMemory: ConversationMemory = {
      ...
      skippedQuestions: [],
      followUpQueue: [],
      elementsToVerify: interviewPreparationContext.expectedSkillsToDemonstrate.map((s: any) => s.id),
      confirmedElements: [],
      unknownElements: [],
      explainability: {
        source: "Voice Interview Engine",
    ...
    const conversationObjectives: ConversationObjective[] = interviewPreparationContext.interviewObjectives.map((obj: any) => ({
      id: obj.id,
    ...
    return {
      ...
      remainingQuestions: interviewPreparationContext.questionQueue.map((q: any) => q.id),
      conversationMemory,
  ...
  static selectNextQuestion(
    ...
    const phaseQuestions = interviewPreparationContext.questionQueue.filter((q: any) => {
      switch (currentPhase) {
        case "Introduction":
        case "Warmup":
    ...
    const nextQuestion = phaseQuestions.find((q: any) => remainingQuestions.includes(q.id));
  ...
  static skipQuestion(
    ...
    // Select next question from remaining questions
    if (context.remainingQuestions.length > 0) {
      const nextQuestionId = context.remainingQuestions[0];
      // Since interview preparation context was removed, we can only track the ID
