...
export class CareerCopilotTransferableSkillsIntelligenceEngine {
  ...
  static assessTransferability(
    ...
    const candidateSkills = candidateProfile.skills.hardSkills.map((s: any) => s.name.toLowerCase());
    const missingSkills = matchingCoreContext.hardSkills.missing.map((s: any) => s.name.toLowerCase());
    ...
    missingSkills.forEach((missingSkill: any) => {
