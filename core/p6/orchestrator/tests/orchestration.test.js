...
describe("P6.5 - Orchestration Guarantees", () => {
    it("O3, O4, O5, O6: should guarantee 1:1 mapping through the pipeline", () => {
        const mockGovernorDecide = vi.fn((msg, state) => {
            return { trustDelta: 0.1 };
        });
        const mockGovernor = { decide: mockGovernorDecide };
        const mockUXCalculate = vi.fn((state, dec, msg) => {
            return {
                text: "Pipeline 1:1",
                delayMs: 100,
                speechRate: 1.0,
        ...
        const mockUX = { calculateUX: mockUXCalculate };
