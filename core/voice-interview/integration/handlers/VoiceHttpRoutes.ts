...
import type { TicketService } from "../security/TicketService.js";
...
export class VoiceHttpRoutes {
  ...
  async postVoiceSession(req: HttpRequest<void>): Promise<HttpResponse<{ ticket: string }>> {
    // Dans un vrai backend, on lirait req.headers.authorization, 
    // on vérifierait le JWT Supabase, et on extrairait l'userId.
    // Pour l'instant, on simule :
    const userId = "simulated-user-id";
    const ticket = this.ticketService.generateTicket(userId);
