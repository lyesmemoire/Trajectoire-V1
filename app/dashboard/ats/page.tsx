...
function ATSDashboardContent() {
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    // Premium Loading simulation (3 seconds)
    await new Promise((r) => setTimeout(r, 3000));

    // Simulation of fetching CV content. In a real app, this would come from a database/storage.
    const mockCvText = `Directed a cross-functional team of 15 engineers to architect a new cloud infrastructure. 
    Reduced latency by 40% and decreased operational costs by 120k€ annually. 
    Orchestrated the strategic migration of legacy systems to a microservices architecture.
    Managed a budget of 500k€ for R&D initiatives.
    Defined the 3-year technical roadmap for the department.`;

    const analyzeText = (text: string) => {
      const content = text.toLowerCase();
      
      // 1. Impact Quantification Detection
      const metrics = (content.match(/%|€|\$|kpi|increase|decrease|growth|saved|reduced|delivered/g) || []).length;
      const impactScore = Math.min(100, 40 + (metrics * 8));

      // 2. Executive Framing Detection
      const strategicVerbs = (content.match(/piloté|structuré|transformé|orchestrated|steered|defined|architected|orchestrated|led|driven/g) || []).length;
      const framingScore = Math.min(100, 40 + (strategicVerbs * 12));

      // 3. Scope & Seniority Signal
      const scopeKeywords = (content.match(/team|budget|p&l|board|cross-functional|transversal|global|department|steering committee/g) || []).length;
      const scopeScore = Math.min(100, 30 + (scopeKeywords * 15));

      // 4. Strategic Alignment (Comparison with Job Description)
      const jobWords = jobDescription.toLowerCase().split(/\W+/).filter(w => w.length > 4);
      const cvWords = content.split(/\W+/).filter(w => w.length > 4);
      const matches = cvWords.filter(w => jobWords.includes(w)).length;
      const alignmentScore = Math.min(100, 50 + (matches * 5));

      // 5. Narrative Coherence
      const coherenceScore = Math.min(100, 60 + (text.length > 500 ? 20 : 10));

      // 6. Posture Detection
      const postures = {
        operational: (content.match(/développé|implémenté|créé|réparé|built|developed|implemented|created|fixed|analysé|testé|configuré/g) || []).length,
        managerial: (content.match(/managé|encadré|coordonné|supervisé|led|managed|coordinated|supervised|recruté|coaché/g) || []).length,
        strategic: (content.match(/défini|orienté|arbitré|planifié|stratégie|defined|steered|architected|strategic|vision|roadmap/g) || []).length,
        transformational: (content.match(/transformation|pivot|restructuration|refonte|transformation|restructured|pivoted|change management|optimisation organisationnelle/g) || []).length,
      };

      let dominantPosture = "Operational";
      let maxCount = -1;
      Object.entries(postures).forEach(([key, count]) => {
        if (count > maxCount) {
          maxCount = count;
          dominantPosture = key.charAt(0).toUpperCase() + key.slice(1);
        }
      });

      return {
        strategicAlignment: alignmentScore,
        impactQuantification: impactScore,
        executiveFraming: framingScore,
        scopeSenioritySignal: scopeScore,
        narrativeCoherence: coherenceScore,
        dominantPosture,
      };
    };

    const dimensionsScores = analyzeText(mockCvText);

    const dimensions = {
      strategicAlignment: {
        score: dimensionsScores.strategicAlignment,
        obs: dimensionsScores.strategicAlignment < 60 
          ? "Décalage stratégique explicite : vos compétences ne s'alignent pas avec les piliers prioritaires du poste." 
          : dimensionsScores.strategicAlignment > 70 
            ? "Strong keyword alignment with the target role." 
            : "Alignment is present but lacks the precision required for a top-tier executive match.",
        impl: dimensionsScores.strategicAlignment < 60 
          ? "Risque élevé d'être écarté dès le premier filtre pour manque d'adéquation stratégique." 
          : dimensionsScores.strategicAlignment > 70 
            ? "High probability of passing initial screening." 
            : "Likely to pass, but may require further clarification during the interview."
      },
      impactQuantification: {
        score: dimensionsScores.impactQuantification,
        obs: dimensionsScores.impactQuantification > 75 
          ? "Excellente crédibilité ROI : vos résultats sont systématiquement appuyés par des preuves chiffrées." 
          : dimensionsScores.impactQuantification < 50 
            ? "Déficit critique de preuves chiffrées : vos accomplissements restent qualitatifs." 
            : "Impact quantified but lacks consistent depth across all experiences.",
        impl: dimensionsScores.impactQuantification > 75 
          ? "Signal fort de performance et de pragmatisme pour le recruteur." 
          : dimensionsScores.impactQuantification < 50 
            ? "Difficulté majeure à justifier une rémunération de niveau Direction sans preuves de valeur." 
            : "ROI signal is present but not yet a competitive advantage."
      },
      executiveFraming: {
        score: dimensionsScores.executiveFraming,
        obs: dimensionsScores.executiveFraming < 50 
          ? "Biais opérationnel marqué : vous communiquez comme un expert technique/manager, pas comme un dirigeant." 
          : dimensionsScores.executiveFraming > 70 
            ? "Narrative successfully emphasizes strategic steering." 
            : "The framing is professional but remains too focused on delivery process.",
        impl: dimensionsScores.executiveFraming < 50 
          ? "Perception d'un profil 'exécution' plutôt que 'stratégie'." 
          : dimensionsScores.executiveFraming > 70 
            ? "Projected as a strategic leader." 
            : "Potential perception of a high-level operational manager rather than a strategic steer."
      },
      scopeSenioritySignal: {
        score: dimensionsScores.scopeSenioritySignal,
        obs: dimensionsScores.scopeSenioritySignal > 80 
          ? "Maturité stratégique évidente : scope de responsabilité et autorité clairement établis." 
          : dimensionsScores.scopeSenioritySignal > 70 
            ? "Clear signals of authority and wide scope." 
            : "Cross-functional exposure is clear, but authority level is ambiguous.",
        impl: dimensionsScores.scopeSenioritySignal > 80 
          ? "Crédibilité immédiate pour piloter des transformations organisationnelles majeures." 
          : dimensionsScores.scopeSenioritySignal > 70 
            ? "Ready for high-level organizational responsibility." 
            : "Potential doubt on the ability to drive organizational change independently."
      },
      narrativeCoherence: {
        score: dimensionsScores.narrativeCoherence,
        obs: dimensionsScores.narrativeCoherence > 70 ? "Trajectory is consistent and logical." : "Trajectory is consistent, though the transition to leadership lacks a pivot point.",
        impl: dimensionsScores.narrativeCoherence > 70 ? "Narrative is easy to follow and convincing." : "The 'jump' to Director may seem abrupt without a clear narrative bridge."
      },
    };

    const overall = Math.round(
      dimensions.strategicAlignment.score * 0.25 +
      dimensions.impactQuantification.score * 0.20 +
      dimensions.executiveFraming.score * 0.20 +
      dimensions.scopeSenioritySignal.score * 0.20 +
      dimensions.narrativeCoherence.score * 0.15
    );

    const recruiterDoubts = [];
    if (dimensionsScores.impactQuantification < 50) {
      recruiterDoubts.push({
        subject: "Déficit de preuves chiffrées",
        dimension: "Impact Quantification",
        reason: "Vos accomplissements sont décrits qualitativement sans indicateurs de performance (KPI) ou métriques financières.",
        impact: "Cela réduit la perception de votre fiabilité et de votre capacité à générer un ROI mesurable.",
        reformulation: "Au lieu de 'Géré une équipe pour livrer X', utilisez 'Livré X, entraînant une augmentation de 12% de l'efficacité et 200k€ d'économies annuelles'.",
      });
    } else if (dimensionsScores.impactQuantification < 70) {
      recruiterDoubts.push({
        subject: "Optimisation de la Proposition de Valeur",
        dimension: "Impact Quantification",
        reason: "Des métriques existent, mais elles ne sont pas assez explicitement liées aux objectifs stratégiques de l'entreprise.",
        impact: "Empêche le candidat d'atteindre une perception de valeur 'top-tier'.",
        reformulation: "Liez chaque résultat chiffré à un impact business global (ex: '...contribuant à l'augmentation de la marge brute de 2%').",
      });
    }

    if (dimensionsScores.executiveFraming < 50) {
      recruiterDoubts.push({
        subject: "Biais Opérationnel Marqué",
        dimension: "Executive Framing",
        reason: "Le langage utilisé met l'accent sur la gestion des processus plutôt que sur la vision stratégique.",
        impact: "Suggère un état d'esprit de 'Manager' plutôt que de 'Directeur'.",
        reformulation: "Au lieu de 'Supervisé la migration de Y', utilisez 'Orchestré la migration stratégique de Y pour s'aligner sur l'objectif de croissance à 3 ans'.",
      });
    } else if (dimensionsScores.executiveFraming < 70) {
      recruiterDoubts.push({
        subject: "Nuancer l'Autorité Exécutive",
        dimension: "Executive Framing",
        reason: "La narration est solide mais pourrait davantage souligner le pilotage à long terme.",
        impact: "Risque d'apparaître trop focalisé sur l'exécution même à un niveau senior.",
        reformulation: "Utilisez des verbes d'action stratégique : 'Arbitré', 'Déterminé', 'Impulsé' plutôt que 'Coordonné'.",
      });
    }

    if (dimensionsScores.strategicAlignment < 60) {
      recruiterDoubts.push({
        subject: "Décalage Stratégique Explicite",
        dimension: "Strategic Alignment",
        reason: "Il existe des lacunes significatives entre vos compétences mises en avant et les exigences stratégiques du poste.",
        impact: "Risque élevé d'être perçu comme un mauvais match pour les besoins spécifiques du rôle.",
        reformulation: "Identifiez les 3 piliers stratégiques de l'offre et réécrivez vos expériences pour utiliser le même lexique décisionnel.",
      });
    }

    const strengths = [
      "Deep technical expertise in React ecosystem",
      "Proven capacity for cross-functional leadership",
    ];
    if (dimensionsScores.impactQuantification > 75) strengths.push("Excellente crédibilité ROI et culture du résultat");
    if (dimensionsScores.scopeSenioritySignal > 80) strengths.push("Maturité stratégique et autorité exécutive confirmée");
    ...
    setResult({
      ...
      dominantPosture: {
        type: dimensionsScores.dominantPosture,
        analysis: {
          Operational: {
            signal: "Forte capacité d'exécution et maîtrise technique.",
            implication: "Risque d'être perçu comme trop 'hands-on' pour un rôle de Direction. Suggère un profil d'Expert plutôt que de Leader.",
            alignment: dimensionsScores.strategicAlignment > 70 ? "Partiellement aligné : l'exécution est là, mais la vision manque." : "Faible alignement avec les attentes stratégiques d'un poste de Direction."
          },
          Managerial: {
            signal: "Aptitude confirmée au pilotage d'équipes et à la coordination.",
            implication: "Profil solide pour la gestion opérationnelle, mais peut manquer de hauteur stratégique (vision long terme).",
            alignment: "Bien aligné pour des rôles de Direction Opérationnelle / Head of."
          },
          Strategic: {
            signal: "Capacité à définir des directions et à arbitrer des choix complexes.",
            implication: "Alignement naturel avec les attentes d'un comité de direction (CODIR).",
            alignment: "Hautement aligné avec les exigences de leadership stratégique."
          },
          Transformational: {
            signal: "Expertise dans le changement d'échelle et la mutation organisationnelle.",
            implication: "Profil extrêmement précieux pour des phases de scale-up, fusion ou retournement.",
            alignment: "Idéal pour des postes de Direction avec un mandat de transformation."
          }
        }[dimensionsScores.dominantPosture],
      },
      recruiterDoubts: recruiterDoubts,
      strategicBlueprint: [
        {
          priority: "High",
          rec: "Pivot from 'Task Ownership' to 'Outcome Orchestration'.",
          leverage: "Shift the focus from what you did to the business value created, changing the power dynamic from 'candidate' to 'solution'."
        },
        {
          priority: "Medium",
          rec: "Establish a 'Strategic Narrative Bridge' for the last 2 years.",
          leverage: "Explicitly link previous operational wins to the strategic requirements of the Director role to eliminate seniority doubt."
        },
        {
          priority: "High",
          rec: "Implement the XYZ Formula across all key achievements.",
          leverage: "Remove all ambiguity regarding impact by anchoring every claim in a metric, forcing the recruiter to acknowledge your ROI."
        }
      ],
      strengths: strengths,
      interviewRisks: [
        "Risk of being pigeonholed as a 'Super-Developer' instead of a Strategic Leader.",
      ],
    });
