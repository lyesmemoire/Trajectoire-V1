...
interface OpenAIRealtimeHealthProps {
  healthData: {
...
export function OpenAIRealtimeHealth({ healthData }: OpenAIRealtimeHealthProps) {
  ...
  return (
    <Card className="bg-white border border-gray-200/60 shadow-sm">
      <CardHeader>
        <CardTitle className="text-gray-900">OpenAI Realtime Health</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="border border-gray-200 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                {getHealthIcon(healthData.audioHealth)}
              </div>
              <div className={`px-2 py-1 rounded-full text-xs font-medium ${getHealthColor(healthData.audioHealth)}`}>
                {healthData.audioHealth}
              </div>
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-gray-200">
            <div className="flex items-center gap-2 text-xs text-gray-600">
              ...
              <span>Overall Health: {healthData.connectionHealth === "healthy" && healthData.streamingHealth === "healthy" && healthData.audioHealth === "healthy" ? "Healthy" : "Degraded"}</span>
            </div>
          </div>
        </div>
      </CardContent>
