...
interface OpenAIRealtimeTranscriptProps {
  transcriptData: {
...
export function OpenAIRealtimeTranscript({ transcriptData }: OpenAIRealtimeTranscriptProps) {
  return (
    <Card className="bg-white border border-gray-200/60 shadow-sm">
      <CardHeader>
        <CardTitle className="text-gray-900">OpenAI Realtime Transcript</CardTitle>
      </CardHeader>
      <CardContent>
              ...
              transcriptData.finalTranscripts.slice(-5).map((transcript, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-medium text-gray-900">{transcript.speaker}</span>
                    <div className="flex items-center gap-1 text-xs text-gray-600">
                      <Clock className="w-3 h-3" />
                      <span>{new Date(transcript.timestamp).toLocaleTimeString()}</span>
                    </div>
                  </div>
                  <div className="text-sm text-gray-900">{transcript.text}</div>
                </m.div>
              ))
            )}
          </div>
