...
interface SpeechStreamingProps {
  streamingData: {
    isActive: boolean;
...
export function SpeechStreaming({ streamingData }: SpeechStreamingProps) {
  return (
    <Card className="bg-white border border-gray-200/60 shadow-sm">
      <CardHeader>
        <CardTitle className="text-gray-900">Speech Streaming</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-600">Buffer: {streamingData.bufferSize} bytes</div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            <div>
              ...
              <div className="text-sm font-medium text-gray-900">{streamingData.chunksReceived}</div>
            </div>
            <div>
              <div className="text-xs text-gray-600 mb-1">Chunks Out</div>
              <div className="text-sm font-medium text-gray-900">{streamingData.chunksSent}</div>
            </div>
            <div>
              <div className="text-xs text-gray-600 mb-1">Bytes In</div>
              <div className="text-sm font-medium text-gray-900">{(streamingData.bytesReceived / 1024).toFixed(2)} KB</div>
            </div>
            <div>
              <div className="text-xs text-gray-600 mb-1">Bytes Out</div>
              <div className="text-sm font-medium text-gray-900">{(streamingData.bytesSent / 1024).toFixed(2)} KB</div>
            </div>
          </div>
