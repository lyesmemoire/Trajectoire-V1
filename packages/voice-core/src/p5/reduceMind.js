export function reduceMind(state, event) {
    switch (event.type) {
        case "TRUST_DELTA": {
            if (event.delta === 0)
                return state;
            const nextTrust = Math.max(0, Math.min(1, state.trust + event.delta));
            if (nextTrust === state.trust)
                return state;
        ...
        case "SUSPICION_DELTA": {
            ...
            if (nextSuspicion === state.suspicion)
                return state;
        ...
        case "PRESSURE_DELTA": {
            ...
            if (nextPressure === state.pressure)
                return state;
            return { ...state, pressure: nextPressure };
        }
        case "EMOTION_SET": {
            if (event.emotion === state.emotion)
                return state;
...
//# sourceMappingURL=reduceMind.js.map