 ...
 *
 * Fournisseurs :
 *  1. ElevenLabs (primaire) — voix naturelle, multilingue
 *  2. OpenAI TTS (fallback) — utilisé si ElevenLabs échoue
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ── 2. RATE LIMIT (fail-closed : on bloque si indisponible) ──
    try {
    ...
    if (ELEVENLABS_API_KEY && ELEVENLABS_VOICE_ID) {
      try {
        ...
        // On continue vers le fallback OpenAI (même crédit réservé)
      }
    }
    ...
    if (!audioBuffer) {
      ...
      try {
        ...
        // Les deux providers ont échoué → rollback
