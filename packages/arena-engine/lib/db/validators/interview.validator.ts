import { z } from "zod";
import type { StandardSessionRow, PremiumSessionRow } from "@/domain/interview.contract";
...
export const StandardSessionSchema = z.object({
  ...
  feedback_json: z.record(z.unknown()).nullable(),
  created_at: z.string(),
...
export function validateStandardSession(data: unknown): StandardSessionRow {
  return StandardSessionSchema.parse(data) as StandardSessionRow;
}
...
export function validatePremiumSession(data: unknown): PremiumSessionRow {
  return PremiumSessionSchema.parse(data) as PremiumSessionRow;
}
