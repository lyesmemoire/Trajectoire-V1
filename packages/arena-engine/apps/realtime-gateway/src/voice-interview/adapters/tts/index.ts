...
export class ChainTTSAdapter implements TTSAdapter {
  ...
  async synthesize(text: string): Promise<ArrayBuffer> {
    const { recordAndInspectTtsUsage } = await import("../../../../../../lib/security/finops-firewall.js");
    const assessment = await recordAndInspectTtsUsage(this.userId, text.length);
    if (!assessment.allowed || assessment.mockFallback) {
