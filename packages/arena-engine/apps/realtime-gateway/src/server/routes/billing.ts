...
export async function registerBillingRoutes(app: FastifyInstance) {
  ...
  app.post("/api/create-portal-session", async (request, reply) => {
    ...
    const { hasPremiumAccess } = await import("../../../../../lib/billing/subscription-types.js");
    ...
    if (!hasPremiumAccess(subscription) || !usageData?.stripe_customer_id) {
      return reply.status(400).send({ error: "No active subscription found or missing customer ID." });
    }
