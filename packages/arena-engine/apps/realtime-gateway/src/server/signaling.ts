...
import { SignalingMessageThrottler } from "../../../../lib/security/ws-message-throttler.js";
...
export async function registerSignaling(app: FastifyInstance) {
  app.get("/api/signal", { websocket: true }, (connection: any) => {
    ...
    const throttler = new SignalingMessageThrottler({
      endpoint: "/api/signal",
    ...
    socket.on("message", async (raw) => {
      try {
        ...
        if (!throttler.consume(type === "pcm" ? "pcm" : "control")) {
          return;
        }
