...
function HiringSignalBadge({ signal }: { signal: CommitteeDecision['hiringSignal'] }) {
  const config = {
    strong_yes: { label: 'Décision très favorable', classes: 'bg-emerald-100 text-emerald-800 ring-emerald-500/30' },
    yes: { label: 'Avis positif', classes: 'bg-green-50 text-green-700 ring-green-600/20' },
    maybe: { label: 'Avis mitigé', classes: 'bg-orange-50 text-orange-700 ring-orange-600/20' },
    no: { label: 'Avis défavorable', classes: 'bg-rose-50 text-rose-700 ring-rose-600/20' }
  };
...
export function CommitteeDecisionReveal({ decision, overallScore, sessionId, cts, delayMs = 1200 }: Props) {
  ...
  return (
    <div className="mt-8 rounded-[2rem] bg-slate-50 p-8 ring-1 ring-slate-200">
      <AnimatePresence mode="wait">
        {phase === 'idle' && (
          ...
          >
            <button
              ...
              className="rounded-2xl bg-slate-900 px-8 py-4 text-sm font-medium
                         text-slate-50 transition-colors hover:bg-slate-800 shadow-xl"
            >
        ...
        {phase === 'deliberating' && (
          ...
          >
            ...
            <p className="text-sm font-bold tracking-widest text-slate-500 uppercase mt-2">Comité en délibération…</p>
          </motion.div>
        )}
        ...
        {phase === 'revealed' && (
          ...
          >
            {decision.hiringSignal === 'strong_yes' && (
              <div className="absolute inset-0 bg-emerald-500/5 blur-3xl rounded-[3rem] -z-10 pointer-events-none" />
            )}
            ...
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 pt-6 border-t border-slate-200">
              ...
              {cts && (
                <div className="bg-slate-50/50 rounded-2xl p-5 border border-slate-100 flex items-center justify-between">
                  <div>
                    ...
                    <p className="font-bold text-slate-800 mt-1">{cts.label}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-black text-slate-900">{cts.score}</p>
                    <p className={`text-sm font-bold ${cts.delta >= 0 ? 'text-emerald-600' : 'text-rose-500'}`}>
              ...
              {(decision.hiringSignal === 'maybe' || decision.hiringSignal === 'no') && (
                <div className="space-y-4">
                  ...
                  <Link 
                    ...
                    className="inline-flex items-center justify-center w-full sm:w-auto px-6 py-3.5 text-sm font-bold text-white bg-slate-900 rounded-xl hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20"
                  >
