...
export class InterNodeComm extends EventEmitter {
  ...
  private reconnectTimeouts: Map<string, TimerHandle> = new Map();
  private reconnectAttempts: Map<string, number> = new Map();
  private heartbeatTimeouts: Map<string, TimerHandle> = new Map();

  private readonly maxReconnectDelay = 30000;
  private readonly heartbeatTimeoutMs = 15000;

  constructor(private readonly timer: ITimer, nodeId: string, listenPort: number, peers: string[]) {
    super();
    this.nodeId = nodeId;
  ...
  private clearHeartbeatTimeout(peerId: string) {
    ...
    if (existing) {
      this.timer.clearTimeout(existing);
