...
import { GlobalStateSnapshot } from "../observability/GlobalStateStore";
...
export class HealthServer {
  ...
  constructor(
    ...
    cacheIntervalMs: number = 500
  ) {
    this.cacheIntervalMs = cacheIntervalMs;
    // Pre‑populate cache
    this.refreshCache();
    // Periodically refresh without blocking event loop
    this.timer.setInterval(() => this.refreshCache(), this.cacheIntervalMs);
  }

  private refreshCache() {
    this.cachedSnapshot = this.world.stateStore.snapshot();
  }
