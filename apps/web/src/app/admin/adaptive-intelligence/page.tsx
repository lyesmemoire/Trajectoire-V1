  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-ink-900 mb-2">Adaptive Intelligence Dashboard</h1>
        <p className="text-ink-600">Vue d'ensemble de l'orchestration de l'intelligence adaptative.</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg border border-ivoire-200 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-2xl font-bold text-ink-900">{orchestratorStats.averageDecisionTime}ms</p>
          </div>
          <div>
            <p className="text-sm text-ink-600">Temps d'Exécution Moyen</p>
            <p className="text-2xl font-bold text-ink-900">{orchestratorStats.averageExecutionTime}ms</p>
          </div>
          <div>
            <p className="text-sm text-ink-600">Moteurs Utilisés</p>
            <p className="text-2xl font-bold text-ink-900">{orchestratorStats.enginesUsed.length}</p>
          </div>
          <div>
            <p className="text-sm text-ink-600">Échecs</p>
            <p className="text-2xl font-bold text-ink-900">{orchestratorStats.failedExecutions}</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {Object.entries(orchestratorStats.commonActions).map(([action, count]) => (
            <div key={action} className="flex justify-between items-center">
              <span className="text-sm text-ink-600 capitalize">{action}</span>
              <span className="text-sm font-medium text-ink-900">{count}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2">
          {Object.entries(orchestratorStats.commonEngines).map(([engine, count]) => (
            <div key={engine} className="flex justify-between items-center">
              <span className="text-sm text-ink-600 capitalize">{engine}</span>
              <span className="text-sm font-medium text-ink-900">{count}</span>
            </div>
          ))}
        </div>
          ...
          {recentDecisions.map((decision) => (
            <div key={decision.id} className="border-b border-ivoire-100 pb-4 last:border-0 last:pb-0">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <p className="text-sm font-medium text-ink-900">{decision.reasoning}</p>
                  <p className="text-xs text-ink-500 mt-1">
                    {new Date(decision.timestamp).toLocaleString()} - {decision.userId}
                  </p>
                </div>
                <span className={`text-xs px-2 py-1 rounded ${
                  decision.priority === "critical" ? "bg-brick-100 text-brick-800" :
                  decision.priority === "high" ? "bg-terracotta-100 text-terracotta-800" :
                  decision.priority === "medium" ? "bg-ink-100 text-ink-800" :
                  "bg-ivoire-100 text-ink-800"
                }`}>
                  {decision.priority}
                </span>
              </div>
              <p className="text-sm text-ink-600">{decision.expectedOutcome}</p>
              <p className="text-xs text-ink-500 mt-1">{decision.actionsCount} actions</p>
            </div>
          ))}
        </div>
      </div>
            ...
            <div key={engine.engine} className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full ${
                  engine.status === "active" ? "bg-forest-500" : "bg-ink-400"
                }`} />
                <span className="text-sm text-ink-600 capitalize">{engine.engine}</span>
              </div>
