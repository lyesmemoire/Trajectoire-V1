...
describe("Interview Decision (End Interview Condition)", () => {
  ...
  it("comp�tences suffisamment couvertes (all PROVEN) -> END_INTERVIEW", () => {
    const state = {
      competencies: [
        { name: "A", status: "PROVEN", evidenceCount: 1, bestScore: 90, missingEvidence: [], attempts: 1, lastEvaluatedAtTurn: 1 },
      ],
  ...
  it("contradiction MEDIUM/HIGH OPEN -> CONTINUE m�me si PROVEN", () => {
    const state = {
      competencies: [
        { name: "A", status: "PROVEN", evidenceCount: 1, bestScore: 90, missingEvidence: [], attempts: 1, lastEvaluatedAtTurn: 1 },
      ],
  ...
  it("garde-fou maximum atteint -> END_INTERVIEW m�me avec NOT_TESTED", () => {
    const state = {
      competencies: [
