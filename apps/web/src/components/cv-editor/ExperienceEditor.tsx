...
export const ExperienceEditor = React.memo(function ExperienceEditor({
  experience, onChange, onRewrite }: {
  experience: Experience
  onChange: (exp: Experience) => void
  onRewrite: () => void
}) {
  return (
    <div className="p-6 rounded-2xl border border-ivoire-200 bg-ivoire-50 space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <input
          ...
          className="p-3 rounded-xl border border-ivoire-200 bg-white"
        />
        <input
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-end">
          ...
          <Button
            ...
            size="sm"
            onClick={onRewrite}
            className="text-ink-600 hover:text-ink-700 gap-1.5 h-8"
          >
        ...
        <textarea
          ...
          className="w-full p-4 min-h-[100px] rounded-xl border border-ivoire-200 bg-white"
        />
        ...
        {experience.bullets.map((bullet, idx) => (
          <div key={idx} className="flex gap-2">
            <input
              ...
              className="flex-1 p-2 rounded-lg border border-ivoire-200 bg-white text-sm"
            />
          </div>
        ))}
      </div>
