  ...
  return (
    <footer className="py-12 border-t border-white/5 bg-[#050816] px-6">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <Link href="#" className="flex items-center gap-2 group">
              <div className="w-8 h-8 bg-gradient-to-br from-ink-700 to-ink-900 rounded-lg flex items-center justify-center transition-transform group-hover:scale-105 duration-200">
                  ...
                  if (isExternal) {
                    return (
                      <Link
                        key={lIdx}
                        href={link.href}
                        className="text-ink-400 hover:text-white text-xs font-semibold block transition-colors"
                      >
                        {link.label}
                      </Link>
                    )
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-8 border-t border-white/5 text-xs font-bold text-ink-500">
