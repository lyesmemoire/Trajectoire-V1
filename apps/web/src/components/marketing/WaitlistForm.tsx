    ...
    <div className="w-full max-w-xl mx-auto">
      <AnimatePresence mode="wait">
        {step === "email" && (
          ...
          >
            <Input
              ...
              className="h-16 rounded-2xl bg-white/[0.03] border-white/10 text-white text-center font-bold text-lg focus:ring-bronze-500"
            />
            <Button
              onClick={() => formData.email && setStep("pressure")}
              className="w-full h-16 rounded-2xl bg-bronze-600 hover:bg-bronze-700 text-white font-black text-lg"
            >
              ...
              {["Tech", "Product", "Consulting", "Leadership", "Autre"].map(
                (opt) => (
                  <button
                    ...
                    className="p-5 rounded-xl bg-white/[0.02] border border-white/10 text-ink-300 hover:bg-ivoire-100/10 hover:border-ink-500 transition-all font-bold"
                  >
                    {opt}
                  </button>
                ),
            ...
            <div className="grid grid-cols-1 gap-3">
              {[
                ...
                <button
                  ...
                  className="p-5 rounded-xl bg-white/[0.02] border border-white/10 text-ink-300 hover:bg-bronze-600/10 hover:border-bronze-500 transition-all font-bold"
                >
                  {opt}
                </button>
              ))}
        ...
        {step === "intent" && (
          ...
          >
            <div className="space-y-4 text-left">
              <label className="text-[10px] font-black text-ink-500 uppercase tracking-widest ml-2">
              ...
              <textarea
                ...
                className="w-full h-32 p-6 rounded-2xl bg-white/[0.02] border border-white/10 text-white outline-none focus:border-bronze-500 resize-none font-medium"
              />
            </div>
            <Button
              onClick={submit}
              disabled={loading || !formData.intentReason}
              className="w-full h-16 rounded-2xl bg-bronze-600 text-white font-black text-lg"
            >
              {loading ? (
        ...
        {step === "done" && (
          ...
          >
            <div className="w-20 h-20 bg-forest-500/10 rounded-[2rem] border border-forest-500/20 flex items-center justify-center text-4xl mx-auto shadow-premium animate-pulse">
              ✅
            </div>
