...
export function SkillProgress({ skills }: SkillProgressProps) {
  ...
  const getScoreColor = (score: number) => {
    if (score >= 0.8) return "bg-forest-500"
    if (score >= 0.6) return "bg-forest-400"
    if (score >= 0.4) return "bg-terracotta-500"
    return "bg-brick-500"
  }
  ...
  return (
    <div className="bg-white/70 backdrop-blur-xl p-6 rounded-lg border border-ivoire-200">
      <h3 className="text-lg font-semibold text-ink-900 mb-4">Progression des compétences</h3>
