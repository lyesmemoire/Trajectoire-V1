...
export const BillingService = {
  ...
  async spendCredits(op: CreditOperation, dbClient?: any): Promise<CreditOperationResult> {
    ...
    if (error) {
      logError("[PRISMA_ERROR] Deduction failed", {
        userId: op.userId,
        amount: op.amount,
        action: op.action,
        error: error.message,
    ...
    const { error: auditLogError } = await supabase.from("credit_usage").insert({
      user_id: op.userId,
      action: op.action,
    ...
    if (auditLogError) {
      logError("[PRISMA_ERROR] Audit log failed (non-blocking)", { error: auditLogError.message });
    }
