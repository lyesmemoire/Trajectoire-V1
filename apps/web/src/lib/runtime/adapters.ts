...
export function analyzeKeywords(cvText: string, jobText: string): KeywordAnalysis {
...
export function mapKeywordAnalysisToProductOutput(analysis: KeywordAnalysis, ): ProductOutput {
  ...
  if (score < 40) {
    risks.push(
      "Correspondance faible : la candidature risque d'être filtrée par un ATS.",
    );
  }
  if (matched.length === 0) {
...
export function interpretScore(score: number): string {
  if (score >= 80) return "Excellent : ton profil colle vraiment à ce poste.";
  if (score >= 65) return "Très bon : tu es un candidat sérieux pour ce poste.";
  if (score >= 45) return "Tu es proche, mais pas encore prêt — quelques ajustements suffisent.";
  if (score >= 25) return "Le potentiel est là : il manque encore des éléments clés.";
 ...
 */
export function estimateImpact(score: number, gapCount: number): number {
 ...
 */
export function buildInterviewPrep(gaps: string[]): InterviewPrep {
  const topGap = gaps[0];
