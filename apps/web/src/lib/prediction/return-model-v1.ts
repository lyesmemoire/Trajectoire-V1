...
export function computeReturnScore(input: ReturnScoreInput): ReturnPrediction {
  ...
  const engagementSignal =
    input.ux.typingSpeed +
    input.behavior.replays * 1.2 -
    input.ux.clickDelayAvg / 1000; // Normalized to seconds

  // 4. Final Aggregation
  const rawScore =
    recoverySignal * 1.4 + engagementSignal * 1.2 - stressSignal * 1.6;

  // 5. Normalization via Sigmoid
  const returnProbability = sigmoid(rawScore / 10);

  // 6. Business Segmentation
  let returnSegment: "HIGH" | "MEDIUM" | "LOW";
  if (returnProbability > 0.7) returnSegment = "HIGH";
  else if (returnProbability > 0.4) returnSegment = "MEDIUM";
  else returnSegment = "LOW";
