...
export class SpeechAnalyzer {
  ...
  private segmentBuffer: SpeechSegment[] = [];
  ...
  async connectStream(stream: MediaStream): Promise<void> {
    if (!this.audioContext || !this.analyserNode) await this.initialize();
  ...
  startVolumeTracking(intervalMs: number = 100): () => void {
    this.volumeHistory = [];
  ...
  private detectSilence(db: number, elapsedMs: number): void {
  ...
  analyzeTranscript(
    ...
    const fillerWordRate =
      totalWords > 0 ? Math.round((fillerWordCount / totalWords) * 100) : 0;
    ...
    const longPauses = silenceSegments.filter(
      (s) => s.endTime - s.startTime >= 2,
    ).length;
    const pauseRatio =
      durationSeconds > 0
  ...
  private rateWPM(wpm: number): SpeedRating {
    if (wpm < WPM_TARGETS.TOO_SLOW) return "too_slow";
    if (wpm < WPM_TARGETS.MIN_IDEAL) return "slow";
    if (wpm > WPM_TARGETS.TOO_FAST) return "too_fast";
    if (wpm > WPM_TARGETS.MAX_IDEAL) return "fast";
    return "ideal";
  }

  private detectFillerWords(text: string): FillerWordOccurrence[] {
  ...
  private analyzeWhisperSegments(segments: any[]): SpeechSegment[] {
    return segments.map((s) => ({
      ...
      wordsPerMinute: Math.round(
        (s.text.split(" ").length / (s.end - s.start)) * 60,
      ),
      isSilence: false,
      text: s.text,
    }));
  ...
  private analyzeSpeedVariation(segments?: any[]): {
    ...
    const stdDev = Math.sqrt(
      wpms.reduce((s, w) => s + Math.pow(w - mean, 2), 0) / wpms.length,
    );
    const cv = stdDev / mean;
    return {
      speedVariation: cv,
      speedVariationRating:
        cv < 0.15 ? "Excellent" : cv < 0.3 ? "Bon" : "Irrégulier",
    };
  }
  ...
  private rateOverall(score: number): OverallRating {
