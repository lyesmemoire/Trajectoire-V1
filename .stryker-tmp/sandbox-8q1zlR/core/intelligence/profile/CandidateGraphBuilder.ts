...
export class CandidateGraphBuilder {
  ...
  static build(input: CandidateGraphInput): CandidateGraph {
    ...
    // Step 5: Generate strengths using InsightEngine
    const strengths = InsightEngine.generateStrengths(overallScore);
    ...
    // Step 12: Generate decision readiness using DecisionEngine
  ...
  private static buildDecisionReadiness(overallScore: number, scores: LiveScores) {
    const decisionEstimation = DecisionEngine.generateDecisionEstimation(overallScore);
    
    return {
      overall: overallScore,
      technicalReadiness: scores.impact,
      behavioralReadiness: (scores.communication + scores.leadership) / 2,
      confidence: decisionEstimation.secondInterviewProbability,
      gaps: [],
    };
  }
