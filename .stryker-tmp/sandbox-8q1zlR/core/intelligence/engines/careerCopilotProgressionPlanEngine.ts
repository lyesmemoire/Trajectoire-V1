...
`;
    ...
    const result = await aiOrchestrator.execute<ProgressionPlanOutput>(
      careerCopilotProgressionPlanV1,
      {
      ...
      },
      {
        provider: "openai",
        model: "gpt-4-turbo",
        promptId: "career-copilot-progression-plan",
        promptVersion: "v1",
        temperature: 0.7,
        maxTokens: 1500,
      }
    ...
    const planEvent: ObservationCreatedEvent = {
      id: `progression-plan-${Date.now()}`,
      timestamp: new Date(),
