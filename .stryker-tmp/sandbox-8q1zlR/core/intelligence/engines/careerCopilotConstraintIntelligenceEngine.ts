...
export class CareerCopilotConstraintIntelligenceEngine {
  ...
  static trackConstraintEvolution({
    ...
    eventBus.publish({
      ...
      id: `constraint_evolution_${Date.now()}`,
      payload: {
        source: "constraint_intelligence",
  ...
  static async analyzeConstraintIntelligence(
    ...
    const applicationHistory = JSON.stringify(analysisResults.filter((a) => a.data.type === "application").slice(-10), null, 2);
    const interviewHistory = JSON.stringify(analysisResults.filter((a) => a.data.type === "interview").slice(-10), null, 2);
    ...
    const result = await aiOrchestrator.execute(
      careerCopilotConstraintIntelligenceV1,
      {
        candidateProfile,
        currentMission,
        recentObservations,
      ...
      },
      {
        ...
        promptId: "career-copilot-constraint-intelligence-v1",
      }
    );

    const constraintIntelligence = result.data as ConstraintIntelligenceOutput;
    ...
    eventBus.publish({
      type: "observation_created",
      id: `constraint_intelligence_${Date.now()}`,
      payload: {
        source: "constraint_intelligence",
        observationType: "general",
