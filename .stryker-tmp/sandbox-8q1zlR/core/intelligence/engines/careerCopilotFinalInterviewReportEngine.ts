...
import { LiveCoachingContext } from "./careerCopilotLiveCoachingIntelligenceEngine";
...
export class CareerCopilotFinalInterviewReportEngine {
  ...
  static generateReport(
    ...
    liveCoachingContext: LiveCoachingContext
  ...
  private static generateDemonstratedStrengths(
    ...
    if (liveAnswerAnalysisContext.strongElements.length > 0) {
      liveAnswerAnalysisContext.strongElements.forEach(strength => {
        strengths.push({
          name: strength,
  ...
  private static generateObservedWeaknesses(
    ...
    if (liveAnswerAnalysisContext.missingElements.length > 0) {
      liveAnswerAnalysisContext.missingElements.forEach(element => {
        weaknesses.push({
          name: element,
          evidence: "Identified during live interview analysis",
          explainability: {
  ...
  private static generateDemonstratedSkills(
    matchingCoreContext: MatchingCoreOutput,
    _liveAnswerAnalysisContext: LiveAnswerAnalysisContext
  ): SkillItem[] {
    const skills: SkillItem[] = [];

    matchingCoreContext.hardSkills.matched.slice(0, 5).forEach(skill => {
      skills.push({
        skill: skill.name,
  ...
  private static generateDetectedContradictions(
    ...
    liveAnswerAnalysisContext.contradictions.forEach(contradiction => {
      contradictions.push({
  ...
  private static generateMissedOpportunities(
    ...
    liveAnswerAnalysisContext.opportunitiesDetected.forEach(opportunity => {
      opportunities.push({
        name: opportunity,
        evidence: "Identified during live interview analysis",
        explainability: {
  ...
  private static generateImprovementPlan(
    gapContext: GapIntelligenceOutput,
    liveAnswerAnalysisContext: LiveAnswerAnalysisContext,
    _liveCoachingContext: LiveCoachingContext
    ...
    if (liveAnswerAnalysisContext.missingElements.length > 0) {
      liveAnswerAnalysisContext.missingElements.slice(0, 2).forEach(element => {
        shortTerm.push({
