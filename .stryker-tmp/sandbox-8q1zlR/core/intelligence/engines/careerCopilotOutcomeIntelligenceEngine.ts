...
export class CareerCopilotOutcomeIntelligenceEngine {
  ...
  private static extractOutcomeData(): any[] {
    return candidateAIBrain.getObservations()
      ...
      .map(obs => ({
        recommendationType: (obs.data as any)?.recommendationType,
        outcome: (obs.data as any)?.outcome,
        timeToResult: (obs.data as any)?.timeToResult,
        effortInvested: (obs.data as any)?.effortInvested,
        timestamp: (obs.data as any)?.timestamp,
      }));
  }
  ...
  static async analyzeOutcomeIntelligence(input: OutcomeIntelligenceInput): Promise<OutcomeIntelligenceOutput> {
    ...
    // Get knowledge evolution for knowledge-aware outcome analysis
    ...
    // Call AI Orchestrator with outcome intelligence prompt
    const result = await aiOrchestrator.execute(
      careerCopilotOutcomeIntelligenceV1,
      {
      ...
      },
      {
        ...
        promptId: "career-copilot-outcome-intelligence-v1",
      }
    );
    ...
    const output: OutcomeIntelligenceOutput = result.data as OutcomeIntelligenceOutput;
    ...
    // Publish outcome analysis event to EventBus
    eventBus.publish({
