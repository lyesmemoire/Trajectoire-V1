...
export class CareerCopilotSuccessIntelligenceEngine {
  ...
  static async analyzeSuccessIntelligence(input: SuccessIntelligenceInput): Promise<SuccessIntelligence> {
    ...
    const confidenceIntelligence = CareerCopilotConfidenceEngine.getCurrentConfidence();
    const metaIntelligence = CareerCopilotMetaIntelligenceEngine.getCurrentMetaIntelligence();

    // Extract data from CandidateAIBrain
    const historicalObservations = candidateAIBrain.getObservations().slice(-15);
    ...
    const marketContext = marketIntelligence
      ? `Growing sectors: ${marketIntelligence.marketTrends.growingSectors.map((t: any) => t.sector).join(", ")}, Emerging skills: ${marketIntelligence.emergingSkills.map((s: any) => s.skill).join(", ")}`
      : "No market context available";

    // Format confidence level
    const confidenceLevel = confidenceIntelligence
      ? `Overall: ${confidenceIntelligence.confidenceLevel}, Confidence: ${confidenceIntelligence.globalConfidence}%`
      : "No confidence level available";

    // Format coherence status
    const coherenceStatus = metaIntelligence
      ? `Global coherence: ${metaIntelligence.globalCoherence}%, Synchronized analyses: ${metaIntelligence.synchronizedAnalyses}/${metaIntelligence.totalAnalyses}`
      : "No coherence status available";

    // Format goal status
    const goalStatus = currentGoals
      ? `Primary: ${currentGoals.primaryGoal?.description}, New goals: ${currentGoals.newGoals?.join(", ") || "none"}`
      : "No goal status available";

    // Extract scenario intelligence from Brain observations
    ...
    const scenarioContext = scenarioObs.length > 0 && scenarioObs[0]
      ? `Recommended scenario: ${(scenarioObs[0].data as any).recommendation?.recommendedScenario || "No recommendation"}, Best scenario: ${(scenarioObs[0].data as any).comparison?.bestScenario || "None"}, Success maximization: ${(scenarioObs[0].data as any).recommendation?.successMaximization || "None"}`
...
`;
    ...
    const result = await aiOrchestrator.execute(
      careerCopilotSuccessIntelligenceV1,
      {
      ...
      },
      {
        provider: "openai",
        model: "gpt-4-turbo",
        promptId: "career-copilot-success-intelligence",
        promptVersion: "v1",
        temperature: 0.7,
        maxTokens: 2500,
      }
    ...
    // Parse the result
    const successIntelligence = JSON.parse(result.data as string) as SuccessIntelligence;
    ...
    eventBus.publish({
      id: `success-intelligence-${Date.now()}`,
      timestamp: new Date(),
