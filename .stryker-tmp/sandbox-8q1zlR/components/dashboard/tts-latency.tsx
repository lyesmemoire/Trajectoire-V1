...
export function TTSLatency({ latencyData }: TTSLatencyProps) {
  ...
  const synthesisStatus = getLatencyStatus(latencyData.synthesisLatency, latencyData.threshold);
  const streamingStatus = getLatencyStatus(latencyData.streamingLatency, latencyData.threshold);
  const totalStatus = getLatencyStatus(latencyData.totalLatency, latencyData.threshold * 2);

  return (
    <Card className="bg-white border border-gray-200/60 shadow-sm">
      <CardHeader>
        <CardTitle className="text-gray-900">TTS Latency</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div>
              <div className="text-xs text-gray-600 mb-1">Synthesis</div>
              <div className={`text-sm font-medium ${getLatencyColor(synthesisStatus)}`}>
                {latencyData.synthesisLatency}ms
              </div>
            </div>
            <div>
              <div className="text-xs text-gray-600 mb-1">Streaming</div>
              <div className={`text-sm font-medium ${getLatencyColor(streamingStatus)}`}>
                {latencyData.streamingLatency}ms
              </div>
            </div>
            <div>
