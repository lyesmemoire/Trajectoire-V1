...
import { Mic, MicOff, Play, Pause, Square, AlertTriangle, CheckCircle } from "lucide-react";

interface TTSSessionProps {
  sessionData: {
    state: string;
    sessionId: string;
...
export function TTSSession({
  ...
  const getStateIcon = (state: string) => {
    switch (state) {
      ...
      case "Paused":
      case "Stopped":
        return <MicOff className="w-4 h-4 text-gray-600" />;
      default:
  ...
  return (
    <Card className="bg-white border border-gray-200/60 shadow-sm">
      <CardHeader>
        <CardTitle className="text-gray-900">TTS Session</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="text-sm font-medium text-gray-900">{sessionData.voice}</div>
            </div>
            <div>
          ...
          <div className="pt-3 border-t border-gray-200">
            <div className="text-xs text-gray-600 mb-2">Format: {sessionData.format} | Sample Rate: {sessionData.sampleRate}Hz</div>
