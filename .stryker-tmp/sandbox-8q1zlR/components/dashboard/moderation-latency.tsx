...
interface ModerationLatencyProps {
  latencyData: {
    textLatency: number;
    imageLatency: number;
    totalLatency: number;
...
export function ModerationLatency({ latencyData }: ModerationLatencyProps) {
  ...
  const textStatus = getLatencyStatus(latencyData.textLatency, latencyData.threshold);
  const imageStatus = getLatencyStatus(latencyData.imageLatency, latencyData.threshold);
  const totalStatus = getLatencyStatus(latencyData.totalLatency, latencyData.threshold * 2);

  return (
    <Card className="bg-white border border-gray-200/60 shadow-sm">
      <CardHeader>
        <CardTitle className="text-gray-900">Moderation Latency</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div>
              ...
              <div className={`text-sm font-medium ${getLatencyColor(textStatus)}`}>
                {latencyData.textLatency}ms
              </div>
            </div>
            <div>
              <div className="text-xs text-gray-600 mb-1">Image</div>
              <div className={`text-sm font-medium ${getLatencyColor(imageStatus)}`}>
                {latencyData.imageLatency}ms
              </div>
            </div>
            <div>
