...
export class ByzantineNode {
  constructor(private nodeId: string, private mode: "honest" | "faulty" | "malicious" = "honest") {}
  ...
  run(): ByzantineNodeResult {
    ...
    if (this.mode === "faulty") {
      ...
      // Generate a pseudo‑random hash (deterministic based on nodeId for reproducibility)
      const crypto = require("crypto");
      rootHash = crypto.createHash("sha256").update(this.nodeId + Date.now().toString()).digest("hex");
      success = false;
    }
