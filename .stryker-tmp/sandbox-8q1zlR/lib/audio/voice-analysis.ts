...
export function analyzeVoiceStress(signals: VoiceSignals): StressAnalysisResult {
  ...
  if (signals.wordCount !== undefined && signals.responseDurationMs !== undefined) {
    ...
    if (wps < 1.8 || wps > 4.5) score += 25;
    else if (wps < 2.2 || wps > 4.0) score += 12;
    weight += 25;
  }
  ...
  if (signals.fillerCount !== undefined && signals.wordCount !== undefined) {
    ...
    if (fillerRatio > 0.12) score += 30;
    else if (fillerRatio > 0.06) score += 15;
    weight += 30;
  }

  // Pauses and hesitation
  if (signals.pauseCount !== undefined) {
    if (signals.pauseCount > 4) score += 20;
    else if (signals.pauseCount > 2) score += 10;
    weight += 20;
  }

  // Hesitation markers
  if (signals.hesitationMarkers !== undefined) {
    if (signals.hesitationMarkers > 3) score += 15;
    else if (signals.hesitationMarkers > 1) score += 8;
    weight += 15;
  }

  // Interruptions (if tracked)
  if (signals.interruptionCount !== undefined) {
    if (signals.interruptionCount > 2) score += 10;
    weight += 10;
  }
