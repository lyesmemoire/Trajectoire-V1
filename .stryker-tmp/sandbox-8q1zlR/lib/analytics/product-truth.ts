...
export interface FeatureValueScore {
  ...
  totalCost: number;
  usageCount: number;
  retentionImpact: number; // % of users who return after using this feature
  retryImpact: number; // average retries after using this feature
  valueScore: number; // The calculated metric: (Retention + Retry) / Cost
}

/**
 * Calculates the real business value of each feature by correlating cost and behavior.
 */
export async function computeFeatureValueScores(): Promise<
  FeatureValueScore[]
> {
  const features = ["interview", "ats", "dna", "replay", "optimize"];
  const results: FeatureValueScore[] = [];
