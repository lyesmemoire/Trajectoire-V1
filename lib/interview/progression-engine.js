...
export async function calculateTrend(userId) {
    ...
    const startScore = history
        .slice(0, 3)
        .reduce((acc, curr) => acc + curr.pressureLevel, 0) / Math.min(3, history.length);
    const endScore = history
        .slice(-3)
        .reduce((acc, curr) => acc + curr.pressureLevel, 0) / Math.min(3, history.length);
    return {
