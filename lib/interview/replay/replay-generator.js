export function generateReplayFromSession(session) {
    const events = [];
    const pressureCurve = [];
    ...
    answers.forEach((ans, idx) => {
        ...
        if (idx > 0 && ans.score > answers[idx - 1].score + 20) {
            events.push({
                ...
                description: "Vous avez repris le contrôle après une question difficile.",
                pressureLevel: ans.pressureAfter,
                coachingAdvice: "Votre capacité à rebondir est un signal fort de leadership.",
            });
        }
    });
    ...
    return {
        ...
        overallCoaching: "Votre performance montre une bonne maîtrise technique mais une fragilité sous pression intense.",
    };
}
function computeArchetype(answers) {
    const avgConfidence = answers.reduce((s, a) => s + (a.confidence || 50), 0) / answers.length;
    if (avgConfidence > 80)
        return "Confident Performer";
    if (avgConfidence < 40)
        return "Stress Reactive";
    return "Analytical Thinker";
}
//# sourceMappingURL=replay-generator.js.map