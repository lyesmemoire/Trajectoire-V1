...
import { chooseStrategy } from "./followup-strategy";
 ...
 */
export async function orchestrateInterviewStep(sessionId, userAnswer, currentQuestion, metrics
// { silenceDuration, _wordCount, _consecutiveHesitations }
    ...
    if (recovery.active) {
        ...
        const analysis = await analyzeAnswer(userAnswer, currentQuestion);
        pressure = Math.min(100, pressure + 5);
        const followup = chooseStrategy(analysis, pressure);
    ...
    const nextQuestion = await generateRecruiterPrompt({
        ...
        analysis: { clarity: 50, specificity: 50, confidence: 50 }, // Simplified for brevity
        strategy,
        userAnswer,
    });
    // 3. Persist State
...
//# sourceMappingURL=interview-orchestrator.js.map