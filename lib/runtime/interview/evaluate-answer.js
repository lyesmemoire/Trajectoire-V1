...
function hasQuantifiedResult(text) {
    return /\d+\s*(%|k€|€|x|fois|jours?|semaines?|mois|heures?|h\b|points?)/i.test(text);
}
...
function buildFeedback(score, star, gapCovered, gap, quantified) {
    const positives = [];
    if (star.situation)
        positives.push("Tu poses bien le contexte.");
    if (star.action)
        positives.push("Tes actions sont explicites.");
    if (star.result)
        positives.push("Tu mentionnes un résultat.");
    if (quantified)
        positives.push("Tu chiffres ton impact — excellent réflexe.");
    if (gapCovered && gap)
        positives.push(`Tu adresses directement « ${gap} ».`);
    const improve = [];
    if (!star.situation)
        improve.push("Plante le décor : où, quand, avec qui ?");
    if (!star.task)
        improve.push("Précise l'objectif ou ta responsabilité.");
    if (!star.action)
        improve.push("Détaille ce que TU as fait concrètement.");
    if (!star.result)
