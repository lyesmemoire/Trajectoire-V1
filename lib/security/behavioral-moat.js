 ...
 */
export function injectBehavioralVariance(basePrompt, sessionId) {
...
export function computeSessionThreatScore(signals) {
    let score = 0;
    if (signals.headlessDetected)
        score += 50;
    if (signals.cloningRisk)
        score += 30;
    if (signals.rapidNavigation)
        score += 20;
    return Math.min(100, score);
}
//# sourceMappingURL=behavioral-moat.js.map