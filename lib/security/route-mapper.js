 ...
 */
export function resolveInternalPath(publicPath, userId) {
    // 1. Check static aliases
    const staticEntry = Object.entries(INTERNAL_TO_VIRTUAL).find(([_, v]) => publicPath === v || publicPath.startsWith(`${v}/`));
    if (staticEntry) {
 ...
 */
export function getPublicUrl(internalPath, userId) {
    const staticAlias = INTERNAL_TO_VIRTUAL[internalPath];
    if (staticAlias)
        return staticAlias;
