...
export function getCTSLabel(score) {
    if (score <= 40)
...
export async function computeAndSaveCTS(userId, sessionId, feedback) {
    try {
        ...
        const { _data: history, error } = await supabase
            ...
            .limit(4);
        if (error) {
            console.error("[CTS] Error fetching history:", error);
            return null;
        }
        ...
        const { error: updateError } = await supabase
            ...
            .eq("id", sessionId);
        if (updateError) {
            console.error("[CTS] Error saving score:", updateError);
        }
        return {
            score: finalCts,
            delta,
...
//# sourceMappingURL=career-trajectory.js.map