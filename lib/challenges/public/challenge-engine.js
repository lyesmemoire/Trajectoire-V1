...
export async function updateChallengeProgress(sessionId, score, pressure, interruptions) {
    const session = await prisma.interviewSession.findUnique({
        where: { id: sessionId },
        select: { challengeEntryId: true, userId: true },
    });
    if (!session?.challengeEntryId)
        return;
...
//# sourceMappingURL=challenge-engine.js.map