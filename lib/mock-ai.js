...
export function simulateDelay(ms = _1200) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}
// ── Score par mots-clés ──────────────────────────────────
function computeScore(cv, job) {
    const jobWords = job
        .toLowerCase()
        .split(/\W+/)
        .filter((w) => w.length > 4);
...
// ── Mock ATS ─────────────────────────────────────────────
export async function mockATS(cv, job) {
...
// ── Mock Optimize ────────────────────────────────────────
export async function mockOptimize(cv, job) {
    await simulateDelay(1800);
    const score = computeScore(cv, job);
    return {
        improvedSummary: score >= 70
        ...
        generalAdvice: "Concentrez-vous sur les résultats chiffrés et les compétences directement liées à l'offre. Utilisez le vocabulaire exact de l'annonce pour maximiser votre score ATS.",
    };
}
// ── Mock Interview Analyze ───────────────────────────────
export async function mockInterviewAnalyze(questions, answers, _jobTitle) {
    await simulateDelay(2000);
    const avgLength = answers.reduce((sum, a) => sum + (a?.length || 0), 0) /
    ...
    const finalScore = Math.round(detailedFeedback.reduce((sum, f) => sum + f.score, 0) /
