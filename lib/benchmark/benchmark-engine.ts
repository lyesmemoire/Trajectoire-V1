 ...
 */
export async function computeBenchmark(userId: _string, metric: "clarity" | "confidence" | "technical", ): Promise<BenchmarkResult> {
  const profile = await prisma.careerProfile.findUnique({ where: { userId } });
  if (!profile) return { percentile: 50, category: "average" };

  const userScore = (profile as unknown)[`${metric}Score`] || 50;
  ...
  const allProfiles = await prisma.careerProfile.findMany({
    select: { [`${metric}Score` as unknown]: true },
  });

  const lowerScores = allProfiles.filter(
    (p) => (p as unknown)[`${metric}Score`] < userScore,
  ).length;
