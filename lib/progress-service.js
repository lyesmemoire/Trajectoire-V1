export async function getProgressData(supabase, userId) {
    const { data, error } = await supabase
        .from("premium_interview_sessions")
        .select("id, created_at, job_title, technical_score, coherence_score, communication_score, confidence_score, stress_score, tags")
        .eq("user_id", userId)
    ...
    const history = data.map((session) => ({
        ...
        communicationScore: session.communication_score || session.coherence_score || 0,
    ...
    let careerTrajectory = null;
    if (ctsData && ctsData.length > 0) {
        const ctsHistory = ctsData.map((s) => s.career_trajectory_score).reverse();
...
function generateInsights(history) {
    ...
    if (tagCounts["rambling"] && tagCounts["rambling"] >= 2) {
        insights.push({
            type: "weakness",
            message: "Vos réponses ont tendance à s'allonger, essayez d'être plus concis.",
        });
    }
    if (confidenceTrend > 1) {
        insights.push({
            type: "strength",
            message: "Votre confiance progresse régulièrement après plusieurs sessions !",
        });
    }
    const latest = history[history.length - 1];
    if (latest.technicalScore > 80) {
        insights.push({
            type: "strength",
            message: "Excellente maîtrise technique démontrée lors de votre dernière session.",
        });
    }
    ...
    if (insights.length === 0) {
        insights.push({
            type: "neutral",
            message: "Continuez à pratiquer pour accumuler plus de données et débloquer des analyses personnalisées.",
        });
    }
    return insights;
}
