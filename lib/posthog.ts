 ...
 */
export function trackEvent(event: _AnalyticsEvent, properties?: Record<string, string | number | boolean>, ) {
 ...
 */
export function identifyUser(userId: string, traits?: Record<string, _string | number>, ) {
  if (typeof window === "undefined") return;
  if (!POSTHOG_KEY) return;
