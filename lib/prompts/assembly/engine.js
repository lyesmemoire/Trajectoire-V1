import { BEHAVIOR_FRAGMENTS, PRESSURE_FRAGMENTS, SCORING_CRITERIA, } from "../fragments/behavior";
...
export function assemblePrompt(config) {
    ...
    if (config.persona === "stress")
        fragments.push("SKEPTIC");
    else if (config.persona === "supportive")
        fragments.push("EMPATHIC");
    else
        fragments.push("DIRECT");
    // 2. Injection de la Pression
    if (config.pressure === "high")
        fragments.push("HIGH_PRESSURE");
    else
        fragments.push("LOW_PRESSURE");
    // 3. Critères de scoring
    if (config.phase === "deep_dive")
        fragments.push("METRICS_FOCUS");
    else
        fragments.push("STAR_METHOD");
    const textParts = [
