...
import { calculatePremiumATSScore, } from "./scoring/premium-engine";
...
import { analyzeTechnicalLeadership, detectInconsistencies, predictInterviewRisks, } from "./behavioral-logic/recruiter-grade";
 ...
 */
export async function processPremiumATSAnalysis(cvBuffer, jobDescription) {
    ...
    const skillMatch = calculateSkillScore(normalizeSkills(jobData.hard_skills), normalizeSkills(cvProfile.hard_skills));
    // 4. Analysis Layers
...
async function analyzeJobOfferIntelligence(text) {
    const { object } = await generateObject({
        model: mistralSmallModel,
        schema: JobIntelligenceSchema,
        temperature: 0.1,
        system: 'Analyze job offer. JSON format: { "title": "", "hard_skills": [], "seniority": "", "min_years": 0 }',
        prompt: text,
    });
    return object;
}
async function extractAdvancedCVProfile(text) {
    const { object } = await generateObject({
        model: mistralSmallModel,
        schema: AdvancedCVSchema,
        temperature: 0.1,
        system: 'Analyze CV. JSON format: { "hard_skills": [], "seniority": 0, "leadership_score": 0, "impact_metrics_score": 0, "years_experience": 0 }',
        prompt: text,
    });
    return object;
}
async function simulateRecruiterFeedback(_cv, _job, score) {
    const { object } = await generateObject({
        model: mistralModel,
        schema: RecruiterFeedbackSchema,
        temperature: 0.1,
        system: 'Act as a picky recruiter. JSON: { "concerns": [], "strengths": [], "rewrites": [{ "original": "", "improved": "" }] }',
        prompt: `Score: ${score}`,
    });
    return object;
}
//# sourceMappingURL=premium-orchestrator.js.map