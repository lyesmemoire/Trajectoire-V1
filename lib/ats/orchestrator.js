...
export async function processATSAnalysis(cvBuffer, jobDescription) {
    // 1. Extraction
    ...
    const skillResult = calculateSkillScore(normalizedJobSkills, normalizedCVSkills);
    ...
    // 5. Enrichment IA
    const feedback = await generateATSFeedback(skillResult.matched, skillResult.missing, finalScore);
    return {
        score: finalScore,
...
async function parseJobOffer(text) {
    const { object } = await generateObject({
        model: mistralSmallModel,
        schema: JobOfferSchema,
        temperature: 0.1,
        system: 'Extrait les compétences techniques requises. JSON format: { "required": [] }',
        prompt: text,
    });
    return object;
}
async function parseCVSkills(text) {
    const { object } = await generateObject({
        model: mistralSmallModel,
