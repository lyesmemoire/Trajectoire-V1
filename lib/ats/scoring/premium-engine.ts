 ...
 */
export function calculatePremiumATSScore(baseMetrics: unknown, behavioralSignals: unknown, ): PremiumATSScore {
  const dimensions = {
  ...
  const overall = Object.keys(dimensions).reduce((acc, key) => {
    return acc + (dimensions as unknown)[key] * (weights as unknown)[key];
  }, 0);
