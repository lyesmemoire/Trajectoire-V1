 ...
 */
export async function computeActivationScore(userId) {
    let score = 0;
    ...
    if (profile.user.CVAnalysis.length > 0)
        score += 20;
    // 3. First Interview Session Started/Completed (40 pts)
    if (profile.user.interviewSessions.length > 0)
        score += 40;
 ...
 */
export async function getActivationTriggers(userId) {
    const score = await computeActivationScore(userId);
    if (score < 40)
        return "UPLOAD_CV_NUDGE";
    if (score < 80)
        return "START_FIRST_INTERVIEW_NUDGE";
    if (score < 100)
        return "VIEW_RESULTS_NUDGE";
    return null;
}
//# sourceMappingURL=activation-engine.js.map