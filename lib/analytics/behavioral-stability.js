 ...
 */
export async function computeBehavioralStability() {
    ...
    const interruptions = events.filter((e) => e.type.startsWith("interruption")).length;
    const freezes = events.filter((e) => e.type === "silence_detected").length;
    return {
        secondSessionRate: totalUsers > 0 ? (usersWithMultipleSessions / totalUsers) * 100 : 0,
 ...
 */
export function interpretSilence(durationSeconds) {
    if (durationSeconds <= 3)
        return "Réflexion Normale";
    if (durationSeconds <= 7)
        return "Tension / Recherche";
    if (durationSeconds <= 15)
        return "Surcharge Cognitive";
    return "Rupture Psychologique";
}
//# sourceMappingURL=behavioral-stability.js.map