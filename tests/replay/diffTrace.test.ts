...
describe("diffTraces", () => {
  ...
  test("Cas 1: Entrées identiques", () => {
    ...
    const activeDiffs = diffs.filter((d: any) => d.leaderChange || d.eventCountChange || d.missing);
  ...
  test("Cas 5: Missing tick", () => {
    ...
    const { diffs: diffs1 } = diffTraces(traceA, traceB);
    ...
    const { diffs: diffs2 } = diffTraces(traceC, traceD);
  ...
  test("Cas 6: Deterministic ordering", () => {
    ...
    const tickIds = diffs.map((d: any) => d.tickId);
