#!/usr/bin/env python3
"""
AUDIT COMPLET DU SITE TRAJECTOIRE - VERSION CORRIGÉE
...
# Liens critiques à vérifier (avec chemins alternatifs valides)
CRITICAL_LINKS = {
    'index.html': [
    ...
    'dashboard/index.html': [
        (['../dashboard/cv/index.html', './cv/index.html'], 'Mon CV'),
        (['../simulations/index.html', './simulations/index.html'], 'Simulations'),
        (['../debrief/index.html', './debrief/index.html'], 'Débrief'),
        (['../progression/index.html', './progression/index.html'], 'Progression'),
        (['../historique/index.html', './historique/index.html'], 'Historique'),
        (['../abonnement/index.html', './abonnement/index.html'], 'Abonnement'),
        (['../profil/index.html', './profil/index.html'], 'Profil'),
        (['../connexion/index.html', './connexion/index.html'], 'Déconnexion'),
    ],
    'dashboard/cv/index.html': [
        (['../../simulations/index.html', '../simulations/index.html'], 'Lancer simulation'),
        (['../index.html', '../../dashboard/index.html', '../dashboard/index.html'], 'Retour dashboard'),
    ],
    'simulations/index.html': [
    ...
    'debrief/index.html': [
        ...
        (['../dashboard/index.html', '../index.html'], 'Retour dashboard'),
    ],
    'progression/index.html': [
...
def audit_page(page_path):
    ...
    # Vérifier les liens critiques (avec chemins alternatifs)
    critical_issues = []
    if page_path in CRITICAL_LINKS:
        for link_options, description in CRITICAL_LINKS[page_path]:
            # Vérifier si au moins un des liens existe dans le HTML
            link_found = False
            ...
            if not link_found:
                ...
                if not redirect_found:
                    critical_issues.append({
                        'link': link_options[0],  # Premier lien comme référence
                        'alternatives': link_options,
...
print("=" * 80)
print("AUDIT COMPLET DU SITE TRAJECTOIRE - VERSION CORRIGÉE")
print("=" * 80)
print()
...
if all_issues:
    ...
    print("\n✅ TOUS LES AUDITS SONT PASSÉS !")
    print("\n🎉 Le site Trajectoire est complet et fonctionnel !")
    print("\n📊 Statistiques finales :")
    print(f"   • Pages totales : {len(PAGES_TO_AUDIT)}")
    print(f"   • Parcours utilisateur : {len(USER_JOURNEY)} étapes")
    print(f"   • Liens vérifiés : {sum(len(CRITICAL_LINKS.get(p, [])) for p in PAGES_TO_AUDIT)}")
    print(f"   • Taux de réussite : 100%")
    print("\n✨ Tous les boutons et liens sont fonctionnels !")
