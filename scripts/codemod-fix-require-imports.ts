...
function fixRequireImportsInFile(filePath: string): boolean {
  try {
    let content = readFileSync(filePath, "utf-8");
    const originalContent = content;
    
    // Replace import x from 'module' with import x from 'module'
    // This is a simple pattern matching, not a full AST transformation
    content = content.replace(/const\s+(\w+)\s*=\s*require\(['"]([^'"]+)['"]\)/g, (match, varName, modulePath) => {
      return `import ${varName} from '${modulePath}'`;
    });
    
    // Replace import {  x  } from 'module' with import { x } from 'module'
    content = content.replace(/const\s*\{([^}]+)\}\s*=\s*require\(['"]([^'"]+)['"]\)/g, (match, destructured, modulePath) => {
...
function main() {
  console.log("Starting codemod to fix require imports...");
  ...
  for (const file of files) {
    if (fixRequireImportsInFile(file)) {
      fixedCount++;
