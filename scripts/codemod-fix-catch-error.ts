#!/usr/bin/env ts-node
/**
 * CODEMOD: Fix catch blocks without error variable
 * 
 * This codemod fixes catch blocks that use error variable without declaring it
 */
...
function processFile(filePath: string): FileResult {
  ...
  try {
    ...
    // Find catch blocks without error variable but using error inside
    const catchWithoutErrorRegex = /}\s+catch\s+\{\s*([^}]*error[^}]*)\}/gs;
    const matches = content.match(catchWithoutErrorRegex);
...
function main() {
  ...
  console.log('🔍 Scanning for catch blocks without error variable...');
  ...
  console.log(`   Files with catch block issues: ${results.length}`);
  console.log(`   Total occurrences: ${totalOccurrences}`);
  console.log(`   Files fixed: ${totalFixed}`);
  
  // Save results to file
  const reportPath = 'C:/Temp/codemod-catch-error-report.json';
