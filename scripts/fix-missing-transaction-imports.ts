#!/usr/bin/env tsx
/**
 * Script pour ajouter les imports Transaction manquants dans les fichiers transaction
 */
...
async function main() {
  console.log('🔧 Correction des imports Transaction manquants');
  ...
  for (const file of files) {
    ...
    // Vérifier si Transaction est utilisé mais non importé
    if (content.includes('Transaction') && !content.includes('import { Transaction }')) {
      // Ajouter l'import après les autres imports ou au début
      const importLine = 'import { Transaction } from "./transaction-contract.js";\n';
