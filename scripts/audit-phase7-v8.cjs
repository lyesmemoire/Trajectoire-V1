  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // Chercher "failed" dans la ligne
        if (line.includes('failed') && line.includes('execution-pipeline')) {
          const parts = line.split('>');
          if (parts.length > 1) {
            detectorTest = parts[parts.length - 1].trim();
          }
        }
        
        // Chercher "expected" ou "to contain"
        if (line.includes('expected') && line.includes('to contain')) {
          assertion = line.trim();
          const match = line.match(/expected '([^']+)' to contain '([^']+)'/);
          if (match) {
            obtained = match[1];
            expected = match[2];
          }
        ...
        // Chercher juste un message d'erreur après "→"
        if (line.includes('→') && !assertion) {
          assertion = line.replace('→', '').trim();
          obtained = assertion;
          expected = 'N/A';
        }
      }
      ...
      // Classification
      if (testError.killed || testError.signal === 'SIGTERM' || cleanOutput.includes('timeout')) {
        status = 'INVALID';
        reason = 'Timeout';
        console.log(`  Status: INVALID (Timeout)\n`);
      } else if (cleanOutput.includes('failed') || cleanOutput.includes('FAIL')) {
        status = 'KILLED';
