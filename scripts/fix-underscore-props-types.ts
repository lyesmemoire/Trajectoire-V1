#!/usr/bin/env tsx
/**
 * Script pour corriger les types _ComponentProps en ComponentProps
 * Remplace _ComponentProps par ComponentProps dans les annotations de type
 */
...
async function main() {
  console.log('🔧 Correction des types _ComponentProps en ComponentProps');
  ...
  for (const file of files) {
    ...
    if (matches) {
      content = content.replace(underscorePropsRegex, (match) => {
        // Remplacer _ComponentProps par ComponentProps
        const corrected = match.substring(1); // Supprimer le premier underscore
        fileCorrections++;
        modified = true;
        return corrected;
      });
