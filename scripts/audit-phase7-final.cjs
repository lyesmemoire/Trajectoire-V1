...
killedMutations.forEach((mutation) => {
  ...
  try {
    ...
    try {
      ...
      // Lire à la fois stdout et stderr
      ...
      if (errorOutput.includes('timeout') || testError.killed || testError.signal === 'SIGTERM') {
        ...
        // Extraire les détails d'assertion
        const lines = errorOutput.split('\n');
        for (let i = 0; i < lines.length; i++) {
          const line = lines[i];
          if (line.includes('Expected')) {
            expected = line.includes('Expected') ? line.split('Expected')[1].trim() : '';
          }
          if (line.includes('Received')) {
            obtained = line.includes('Received') ? line.split('Received')[1].trim() : '';
          }
          if (line.includes('AssertionError')) {
            assertion = line.trim();
          }
          if (line.includes('FAIL') && line.includes('execution-pipeline')) {
            detectorTest = line.split('>').pop()?.trim() || 'Test détecté';
          }
        }
