#!/usr/bin/env tsx
/**
 * Script pour corriger tous les imports en double de ReportInput
 */
...
async function main() {
  console.log('🔧 Correction des imports en double de ReportInput');
  ...
  for (const file of files) {
    ...
    // Supprimer les imports ReportInput depuis @/core/p7/contracts/report.contract (chemin incorrect)
    const wrongImportRegex = /import\s+{\s*ReportInput\s*}\s+from\s+['"]@\/core\/p7\/contracts\/report\.contract['"]/g;
