  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // Extraire le nom du test (ligne avec ×)
        ...
        // Extraire l'assertion (ligne suivante avec →)
        if (line.includes('→')) {
          assertion = line.trim();
          // Format: "→ expected 'X' to contain 'Y'"
          const match = line.match(/→ expected '([^']+)' to contain '([^']+)'/);
          if (match) {
            obtained = match[1]; // Ce qui a été reçu
            expected = match[2]; // Ce qui était attendu
          }
          // Format alternatif: "→ X" (juste un message d'erreur)
          else if (line.includes('→ ')) {
            obtained = line.replace('→', '').trim();
            expected = 'N/A';
          }
        }
      }
      ...
      } else if (errorOutput.includes('×') || errorOutput.includes('Failed Tests')) {
        status = 'KILLED';
