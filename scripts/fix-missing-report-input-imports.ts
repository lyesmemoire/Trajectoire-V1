#!/usr/bin/env tsx
/**
 * Script pour ajouter les imports ReportInput manquants dans les fichiers report
 */
...
async function main() {
  console.log('🔧 Correction des imports ReportInput manquants dans report');
  ...
  for (const file of files) {
    ...
    // Vérifier si ReportInput est utilisé mais non importé
    if (content.includes('ReportInput') && !content.includes('import { ReportInput }')) {
      // Ajouter l'import après les autres imports ou au début
      const importLine = 'import { ReportInput } from "../report-contract.js";\n';
