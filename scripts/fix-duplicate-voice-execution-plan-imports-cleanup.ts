#!/usr/bin/env tsx
/**
 * Script pour nettoyer les imports en double de VoiceExecutionPlan
 */
...
async function main() {
  console.log('🔧 Nettoyage des imports en double de VoiceExecutionPlan');
  ...
  for (const file of files) {
    ...
    // Supprimer les imports en double de VoiceExecutionPlan
    const importRegex = /import\s+{([^}]+)}\s+from\s+["']\.\/voice-contract\.js["'];?\n?/g;
    const imports = [];
    let match;
    ...
    if (imports.length > 1) {
      ...
      const mergedImport = `import { ${Array.from(allItems).join(', ')} } from "./voice-contract.js";\n`;
