...
// Mutations finales sur des constantes de configuration
const finalMutations = {
  'execution-context': [
    ...
    },
    {
      type: 'F',
      line: 31,
      description: 'Change default maxFrames from undefined to 99',
      original: 'this.callFrames = new CallFrameManager();',
      mutation: 'this.callFrames = new CallFrameManager(99);'
    }
  ],
  'memory-manager': [
    {
    ...
    },
    {
      type: 'F',
      line: 45,
      description: 'Change default enableProtection to false',
      original: 'enableProtection: options.enableProtection !== false,',
      mutation: 'enableProtection: options.enableProtection === false,'
    },
    {
      type: 'F',
      line: 46,
      description: 'Change default enableTracking to false',
      original: 'enableTracking: options.enableTracking !== false,',
      mutation: 'enableTracking: options.enableTracking === false,'
    }
  ],
  'instruction-cache': [
    {
    ...
    },
    {
      type: 'F',
      line: 47,
      description: 'Change maxSize in statistics initialization',
      original: 'maxSize: this.maxSize,',
      mutation: 'maxSize: this.maxSize - 1,'
    }
  ],
  'instruction-fetch': [
    {
    ...
    },
    {
      type: 'F',
      line: 31,
      description: 'Change default cacheEnabled to false',
      original: 'this.cacheEnabled = options.enabled !== false;',
      mutation: 'this.cacheEnabled = options.enabled === false;'
    }
  ],
  'instruction-decode': [
    {
    ...
    },
    {
      type: 'I',
      line: 113,
      description: 'Change operand separator from comma to space',
      original: 'const operands = instruction.operands.map(op => String(op)).join(\', \');',
      mutation: 'const operands = instruction.operands.map(op => String(op)).join(\' \');'
    }
  ],
  'instruction-execute': [
    {
    ...
    },
    {
      type: 'F',
      line: 150,
      description: 'Change division from floor to ceil',
      original: 'stack.push(Math.floor(a / b));',
      mutation: 'stack.push(Math.ceil(a / b));'
    }
  ],
  'rollback-manager': [
    {
    ...
    },
    {
      type: 'F',
      line: 36,
      description: 'Change default autoSnapshot to false',
      original: 'autoSnapshot: options.autoSnapshot !== false,',
      mutation: 'autoSnapshot: options.autoSnapshot === false,'
    }
  ],
  'thread-manager': [
    {
    ...
    },
    {
      type: 'F',
      line: 47,
      description: 'Change default schedulingPolicy to FIFO',
      original: 'schedulingPolicy: options.schedulingPolicy || SchedulingPolicy.ROUND_ROBIN,',
      mutation: 'schedulingPolicy: options.schedulingPolicy || SchedulingPolicy.FIFO,'
    }
  ],
  'execution-pipeline': [
    {
    ...
    },
    {
      type: 'I',
      line: 43,
      description: 'Change initial cycles to 1',
      original: 'cycles: 0,',
      mutation: 'cycles: 1,'
    }
  ]
...
fs.writeFileSync('c:/Trajectoire/scripts/mutations-identified.json', JSON.stringify(finalMutations, null, 2));
console.log('Final mutations saved to mutations-identified.json');
