...
console.log('=== Audit de régression fonctionnelle - execution-pipeline (DEBUG MODE) ===\n');
...
regressions.forEach((regression, index) => {
  ...
  try {
    ...
    console.log(`Ligne: ${regression.line}`);
    console.log(`Original: "${regression.original}"`);
    console.log(`Mutated: "${regression.mutated}"`);
    
    const mutatedLines = [...originalLines];
    const lineIndex = regression.line - 1;
    
    if (lineIndex >= 0 && lineIndex < mutatedLines.length) {
      const originalLine = mutatedLines[lineIndex];
      if (regression.mutated === '') {
        // Supprimer la ligne
        mutatedLines.splice(lineIndex, 1);
        patchApplied = `DELETE line ${regression.line}: "${originalLine}"`;
      } else {
        mutatedLines[lineIndex] = regression.mutated;
        patchApplied = `REPLACE line ${regression.line}: "${originalLine}" -> "${regression.mutated}"`;
      }
      fs.writeFileSync(filePath, mutatedLines.join('\n'));
      console.log(`Patch appliqué: ${patchApplied}`);
    } else {
      throw new Error('Ligne hors limites');
    }
    
    // Vérifier le fichier après mutation
    const contentAfterMutation = fs.readFileSync(filePath, 'utf8');
    const linesAfterMutation = contentAfterMutation.split('\n');
    console.log(`Lignes après mutation: ${linesAfterMutation.length}`);
    
    // Compiler
    console.log(`\n--- Compilation ---`);
    console.log(`Commande: cd c:/Trajectoire && npx tsc --noEmit`);
    try {
      ...
      console.log(`Erreur: ${compileError.message?.substring(0, 200)}`);
    ...
    // Exécuter tests
    console.log(`\n--- Exécution des tests ---`);
    console.log(`Commande: cd c:/Trajectoire && npx vitest run ${testPath}`);
    ...
    try {
      ...
      if (testError.killed || testError.signal === 'SIGTERM' || errorOutput.includes('timeout')) {
        ...
        processKilled = testError.killed;
        status = 'BUILD ERROR';
    ...
    // Restaurer
    console.log(`\n--- Restauration ---`);
    patchRestored = `RESTORE to original content`;
    fs.writeFileSync(filePath, originalContent);
    console.log(`Patch restauré: ${patchRestored}`);
    
    // Vérifier le fichier après restauration
    const contentAfterRestore = fs.readFileSync(filePath, 'utf8');
    if (contentAfterRestore !== originalContent) {
      console.log('ERREUR: Le fichier n\'a pas été correctement restauré');
      console.log(`Longueur originale: ${originalContent.length}`);
      console.log(`Longueur après restauration: ${contentAfterRestore.length}`);
      process.exit(1);
    }
    console.log(`Fichier restauré: OK (contenu identique)`);
    ...
    console.log(`Diff git après: ${diffAfter ? 'NON VIDE - ARRET' : 'VIDE - OK'}`);
    ...
    console.log(`Timeout: ${timeoutOccurred}`);
    console.log(`Process killed: ${processKilled}`);
    ...
    results.push({
      ...
      patchApplied,
      patchRestored,
      diffBefore: diffBefore ? 'NON VIDE' : 'VIDE',
      diffAfter: diffAfter ? 'NON VIDE' : 'VIDE',
      compilation,
      testsExecuted,
      testsPassed,
      testsFailed,
...
if (finalDiff) {
  console.log('ERREUR: Le dépôt n\'est pas propre à la fin. Diff:');
...
fs.writeFileSync(reportDir + 'regression-debug-results.json', JSON.stringify(results, null, 2));

console.log(`\nRésultats détaillés sauvegardés dans: ${reportDir}regression-debug-results.json`);
