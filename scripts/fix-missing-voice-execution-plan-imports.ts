#!/usr/bin/env tsx
/**
 * Script pour ajouter les imports VoiceExecutionPlan manquants dans les fichiers voice
 */
...
async function main() {
  console.log('🔧 Correction des imports VoiceExecutionPlan manquants');
  console.log('📁 Répertoire:', WEB_DIR);
  console.log('');

  const files = await glob('core/p6/voice/*.ts', {
    cwd: WEB_DIR,
    absolute: true,
  });
  ...
  for (const file of files) {
    ...
    // Vérifier si VoiceExecutionPlan est utilisé mais non importé
    if (content.includes('VoiceExecutionPlan') && !content.includes('import { VoiceExecutionPlan }')) {
      // Ajouter l'import après les autres imports ou au début
      const importLine = 'import { VoiceExecutionPlan } from "./voice-contract.js";\n';
