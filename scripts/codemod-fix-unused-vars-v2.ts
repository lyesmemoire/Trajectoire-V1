...
const EXCLUDE_DIRS = [".git", "node_modules", ".next", "dist", "build", ".stryker-tmp", "BLUEPRINT_GENERATED", "BLUEPRINT_MULTI_LANG_GENERATED"];
...
function fixUnusedVarsInFile(filePath: string): boolean {
  try {
    ...
    // Pattern: const { x, y } = z where x or y is not used
    content = content.replace(/const\s*\{([^}]+)\}\s*=\s*([^;]+);/g, (match: string, destructured: string, source: string) => {
      ...
      const fixedItems: string[] = items.map((item: string) => {
        // Check if this item is used later in the file
...
function main() {
  console.log("Starting codemod to fix unused variables v2...");
