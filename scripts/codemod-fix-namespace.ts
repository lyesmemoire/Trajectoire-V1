...
function fixNamespaceInFile(filePath: string): boolean {
  try {
    let content = readFileSync(filePath, "utf-8");
    const originalContent = content;
    
    // Replace module X { ... } with module X { ... }
    content = content.replace(/namespace\s+(\w+)\s*\{/g, "module $1 {");
...
function main() {
  console.log("Starting codemod to fix no-namespace issues...");
