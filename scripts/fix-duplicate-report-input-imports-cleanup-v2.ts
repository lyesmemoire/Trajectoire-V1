#!/usr/bin/env tsx
/**
 * Script pour nettoyer les imports en double de ReportInput (version générique)
 */
...
async function main() {
  console.log('🔧 Nettoyage des imports en double de ReportInput (version générique)');
  ...
  for (const file of files) {
    ...
    // Supprimer les imports en double de ReportInput (chemin relatif générique)
    ...
    if (imports.length > 1) {
      ...
      let pathToUse = imports[0].path;
      
      imports.forEach(imp => {
        imp.items.forEach(item => allItems.add(item));
        if (imp.path.length > pathToUse.length) {
          pathToUse = imp.path; // Utiliser le chemin le plus spécifique
        }
      });
      
      const mergedImport = `import { ${Array.from(allItems).join(', ')} } from "${pathToUse}report-contract.js";\n`;
